...
<div style='width: 100%'; align='center'><h2><b>PERMINTAAN PEKERJAAN</b></h2></div>
    <table class='table table-bordered centerTable'>
        <thead>
            <tr class='label-info'>
                <th colspan='6' {$header_keterangan}>Keterangan Permintaan Pekerjaan</th>
            </tr>
        </thead>
        <tbody>
            <tr>
              <td {$header_pp}><b>Kode</b></td>
              <td {$content_pp} >{$listPp[0]['PK_KODE']}</td>
            ...
            <tr>
              <td {$header_pp}><b>Status PK</b></td>
              <td>{$listPp[0]['STATUS_PK_NAMA']}</td>
              <td {$header_pp}><b>Kategori PK</b></td>
              <td>{$listPp[0]['KATEGORI_PK_NAMA']}</td>
              <td {$header_pp}><b>&nbsp</b></td>
              <td>&nbsp;</td>
            </tr>
            <tr>
                <td {$header_pp}><b>Tanggal Buat</b></td>
                <td>{$listPp[0]['PK_TANGGAL']}</td>
                <td {$header_pp}><b>Tanggal Validasi</b></td>
                <td>{$listPp[0]['PK_VALIDATED']}</td>
...
<div class="col-md-12">
    <div class="widget wgreen">
        <div class="widget-head">
            <div class="pull-left">Detail PK</div>
        ...
        <div class="widget-content" style="padding: 10px">
            <div style="width: 49%; display: inline-block;" align="left">
                <div ng-if="displayField(['pp_admin'])">
                    ...
                    <a href="<?= base_url() ?>p-upah/pk/printPDF/<?= $listPp[0]['PK_ID'] ?>"><img src="<?= base_url(); ?>assets/default/img/icons/pdf.png" class="icons" title="Export to PDF Document" /></a>
                </div>
            </div>
            ...
            <form  id="exportForm" style="display: none" action="<?= base_url() . $this->uri->segment(1, 0) . '/' . $this->uri->segment(2, 0) . '/export' ?>" method="post">
                <input type="hidden" name="FILENAME" id="filename" value="<?= date('d M Y') . " " . str_replace("/", "-", $listPp[0]['PK_KODE']) ?>" />
