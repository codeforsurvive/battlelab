...
<div class="col-md-12">
    <div class="widget wgreen">
        <div class="widget-head">
            <div class="pull-left">Edit Payment</div>
            ...
            <div class="page-tables">
                <div class="table-responsive">
                    <form id="formBPM" action="<?= base_url() . $this->uri->segment(1, 0) . '/' . $this->uri->segment(2, 0) ?>/edit" method="post" class="form-horizontal" role="form">
                        ...
                        <input type="hidden" name="id_payment" id="id_payment" value="<?= $payment['PAYMENT_ID'] ?>"/>
                        <input type="hidden" name="id_invoice" id="id_invoice" value="<?= $payment['INVOICE_ID'] ?>"/>
                        <input type="hidden" name="id_lpu" id="id_lpu" value="<?= $payment['LPU_ID'] ?>"/>
                        <div class="form-group">
                            ...
                            <div class="col-lg-5">
                                <select name="main_project" id="select_main_project" class="form-control" onchange="">
                                    ...
                                    <?php
                                    foreach ($listMainProject as $MainProject) {
                                        echo '<option ' . ($MainProject['MAIN_PROJECT_ID'] == $my_project['MAIN_PROJECT_ID'] ? ' selected=""' : '') . ' value="' . $MainProject['MAIN_PROJECT_ID'] . '">' . $MainProject['MAIN_PROJECT_KODE'] . ' ' . $MainProject['MAIN_PROJECT_NAMA'] . '</option>';
                                    }
                                    ?>    
                                </select>
                            </div>
                            ...
                            <div class="col-lg-5">
                                <select  id="select_project" class="form-control" onchange="">
                                    ...
                                    foreach ($list_project as $pro) {
                                        echo '<option value="' . $pro['PROJECT_ID'] . '" ' . ($pro['PROJECT_ID'] == $my_project['PROJECT_ID'] ? ' selected=""' : '') . ' >' . $pro['PROJECT_KODE'] . ' ' . $pro['PROJECT_NAMA'] . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                            ...
                            <div class="col-lg-5">
                                <select name="rab" id="select_rab" class="form-control" onchange="">
                                    ...
                                    foreach ($list_rab as $rab) {
                                        echo '<option value="' . $rab['RAB_ID'] . '" ' . ($my_rab['RAB_ID'] == $rab['RAB_ID'] ? ' selected=""' : '') . '>' . $rab['RAB_KODE'] . ' ' . $rab['RAB_NAMA'] . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-5">
                                <select name="jenis_payment" id="select_jenis_payment" class="form-control" onchange="">
                                    <option <?php if ((int) $payment['INVOICE_ID'] > 0) echo ' selected="" ' ?> value="material">Material</option>
                                    <option <?php if ((int) $payment['LPU_ID'] > 0) echo ' selected="" ' ?> value="upah">Upah</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group" id="div_select_supplier" <?php if ((int) $payment['INVOICE_ID'] == 0) echo 'style="display:none"' ?>>
                            <label class="col-lg-2 control-label">Supplier</label>
                            <div class="col-lg-5">
                                <select name="supplier" id="select_supplier" class="form-control" onchange="">

                                    <?php
                                    foreach ($list_supplier as $spy) {
                                        echo '<option value="' . $spy['SUPPLIER_ID'] . '" ' . ($spy['SUPPLIER_ID'] == $payment['SUPPLIER_ID'] ? ' selected=""' : '') . '>' . $spy['SUPPLIER_KODE'] . ' ' . $spy['SUPPLIER_NAMA'] . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-2">
                                <button type="button" class="pull-left btn btn-sm btn-success saveForm" onclick="" id="button_show_stok_barang" style="">Pilih Invoice</button>
                            </div>
                        </div>
...
if ((int) $payment['INVOICE_ID'] > 0) {
    echo 'pilihInvoice(' . (int) $payment['INVOICE_ID'] . ');';
} else if ((int) $payment['LPU_ID'] > 0) {
    echo 'pilih_lpu(' . (int) $payment['LPU_ID'] . ');';
}
?>
    });
</script>
