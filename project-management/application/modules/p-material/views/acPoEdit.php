<script>
	...
	function gantiPajak(){
		...
		$('td[id*=row_hargapajak_]').each(function() {
			var id = $(this).attr('id').replace('row_hargapajak_','');
			console.log(id);
			var volpo = parseFloat($('#row_volpo_' + id).text());
			var harga_awal = $('#row_hargaawal_' + id + ' .number').text().formatFloat();
			var harga_diskon = $('#row_hargadiskon_' + id + ' .number').text().formatFloat();
			var harga_after_diskon = harga_awal - harga_diskon;
			var harga_pajak = 0;
			var harga_final = 0;
			var harga_net = 0;
			if(pilih_pajak!='3'){
			...
			$('#row_hargapajak_' + id).html('' + harga_pajak.formatMoney(2) + '');
			$('#row_harganet_' + id).html('' + harga_net.formatMoney(2) + '');
			$('#row_hargafinal_' + id).html('' + (harga_final).formatMoney(2) + '');
		});
	}
</script>
<!-- Page heading -->
...
<div class="col-md-12">
    <div class="widget wgreen">
        <div class="widget-head">
            <div class="pull-left">Edit PO</div>
...
$selected = '';
if($i[mMainProject::ID]==$PO[mMainProject::ID]) $selected = 'selected';
...
$selected = '';
if($i[mProject::ID]==$PO[mProject::ID]) $selected = 'selected';
echo "<option ".$selected." value='" . $i[mProject::ID] . "'>" . $i[mProject::NAMA] . "</option>";
						...
						<div class="form-group" id="rab_form">
							...
							<div class="col-lg-5">
								<select class="form-control" id="rab" name="rab" disabled>
									<option disabled selected> -- pilih RAB -- </option>
									<?php foreach ($listRAB as $i) {
$selected = '';
if($i[mRab::ID]==$PO[mRab::ID]) $selected = 'selected';
...
$selected = '';
if($i[mTop::ID]==$PO[mTop::ID]) $selected = 'selected';
echo "<option ".$selected." value='" . $i[mTop::ID] . "'>" . $i[mTop::VALUE] . " | " . $i[mTop::DESCRIPTION] . "</option>";
} ?>
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="<?php if($PO[mKategoriPajak::ID]==3) echo 'col-lg-5'; else echo 'col-lg-3'; ?>" id="pilih_pajak_form">
								<select class="form-control" id="pilih_pajak" name="pilih_pajak" onchange="pilihPajak();">
								<?php foreach ($listKategoriPajak as $i) {
$selected = '';
if($i[mKategoriPajak::ID]==$PO[mKategoriPajak::ID]) $selected = 'selected';
echo "<option ".$selected." value='" . $i[mKategoriPajak::ID] . "'>" . $i[mKategoriPajak::NAMA] . "</option>";
} ?>
								</select>
							</div>
							<div class="col-lg-2" id="pilih_nilai_pajak_form" <?php if($PO[mKategoriPajak::ID]==3) echo 'style="display: none"'; ?>>
								<select class="form-control" id="pajak" name="pajak" onchange="gantiPajak();">
<?php foreach ($listPajak as $i) {
$selected = '';
if($i[mPajak::ID]==$PO[mPajak::ID]) $selected = 'selected';
echo "<option ".$selected." value='{\"ID\": \"" . $i[mPajak::ID] . "\", \"VALUE\": \"" . $i[mPajak::VALUE] . "\"}'>" . $i[mPajak::NAMA] . " | " . $i[mPajak::VALUE] . "% </option>";
} ?>
								</select>
							</div>
						</div>
                            ...
                            <div class="col-lg-5">
                                <select name="GUDANG_ID" id="GUDANG_ID" class="form-control">
                                    <?php for ($j = 0; $j < sizeof($listGudang); $j++): 
										$selected = '';
										if($listGudang[$j]['GUDANG_ID']==$PO['GUDANG_ID']) $selected = 'selected';	
									?>
                                        <option <?php echo $selected; ?> value="<?= $listGudang[$j]['GUDANG_ID'] ?>"><?= $listGudang[$j]['GUDANG_NAMA'] ?></option>
                                    <?php endfor; ?>    
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-5">
                                <select name="SUPPLIER_ID" id="SUPPLIER_ID" class="form-control">
                                    <?php for ($j = 0; $j < sizeof($listPengguna); $j++): 
										$selected = '';
										if($listPengguna[$j]['SUPPLIER_ID']==$PO['SUPPLIER_ID']) $selected = 'selected';	
									?>
                                        <option <?php echo $selected; ?> value="<?= $listPengguna[$j]['SUPPLIER_ID'] ?>"><?= $listPengguna[$j]['SUPPLIER_NAMA'] ?></option>
                                    <?php endfor; ?>    
                                </select>
                            </div>
                        </div>
						<div class="form-group">
                            ...
                            <div class="col-lg-5">
                                <select name="KATEGORI_PP_ID" id="KATEGORI_PP_ID" class="form-control" disabled>
                                    <?php for ($j = 0; $j < sizeof($listKategoriPP); $j++): 
										$selected = '';
										if($listKategoriPP[$j]['KATEGORI_PP_ID']==$PO['KATEGORI_PP_ID']) $selected = 'selected';
									?>
                                        <option <?php echo $selected; ?> value="<?= $listKategoriPP[$j]['KATEGORI_PP_ID'] ?>"><?= $listKategoriPP[$j]['KATEGORI_PP_NAMA'] ?></option>
                                    <?php endfor; ?>    
                                </select>
                            </div>
                        </div>
						<div class="form-group">
							...
							<div class="col-lg-5">
								<textarea class="form-control" id="alamat" name="alamat" placeholder="alamat pengiriman"><?= $PO['ALAMAT_PENGIRIMAN'] ?></textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-lg-5">
								...
								<div class="col-lg-9" style="margin-bottom: 10px">
									<input type="text" class="form-control" id="nama_cp" name="nama_cp" placeholder="nama CP" value="<?= $PO['NAMA_CP'] ?>">
								</div>
								<label class="col-lg-3 control-label">No. Telp/HP</label>
								<div class="col-lg-9">
									<input type="text" class="form-control" id="telp_cp" name="telp_cp" placeholder="no telepon CP" value="<?= $PO['TELP_CP'] ?>">
								</div>
							</div>
						</div>
...
<script>
	...
	$(document).ready(function() {
		...
		if (res.length > 0) {
			...
			for (var i = 0; i < res.length; i++) {
				var id = res[i]["BARANG_ID"] + '_' + res[i]["PP_ID"];
				var num = i+1;
				var usediskon = 0;
				if(parseFloat(res[i]["HARGA_DISKON"])>0) usediskon = 1;
				flag[id] = false;
				$(".listBahan").append(
					"<tr id='row_" + id + "'>" +
					"<td style='display:none' id='row_id_" + id + "'>" + id + "</td>" +
					"<td id='row_no_" + id + "'>" + num + "</td>" +
					"<td id='row_kode_" + id + "'>" + res[i]["BARANG_KODE"] + "</td>" +
					"<td id='row_nama_" + id + "'>" + res[i]["BARANG_NAMA"] + "</td>" +
					"<td id='row_volpp_" + id + "'>" + res[i]["VOLUME_PP"] + "</td>" +
					"<td id='row_volpo_" + id + "'>"+ res[i]["VOLUME_PO"] +"</td>" +
					"<td id='row_volsisa_" + id + "'>" + (parseFloat(res[i]["VOLUME_PP_SISA"])-parseFloat(res[i]["VOLUME_PO"])) + "</td>" +
					"<td style='display:none' id='row_volsisatemp_" + id + "'>" + parseFloat(res[i]["VOLUME_PP_SISA"]) + "</td>" +
					"<td id='row_kodepp_" + id + "'>" + res[i]["PP_KODE"] + "</td>" +
					"<td style='display:none' id='row_idpp_" + id + "'>" + res[i]["PP_ID"] + "</td>" +
					"<td id='row_hargasatuan_" + id + "' align='right'>" + parseFloat(res[i]["HARGA_MATERI_PO"]).formatMoney(2) + "</td>" +
					"<td style='display:none' id='row_hargasatuantemp_" + id + "'>" + res[i]["HARGA_DETAIL"] + "</td>" +
					"<td style='display:none' id='row_hargaawal_" + id + "' align='right'>" + (parseFloat(res[i]["HARGA_AWAL"])).formatMoney(2) + "</td>" +
					"<td style='display:none' id='row_hargadiskon_" + id + "' align='right'>" + (parseFloat(res[i]["HARGA_DISKON"])).formatMoney(2) + "</td>" +
					"<td style='display:none' id='row_usediskon_" + id + "'>"+usediskon+"</td>" +
					"<td style='display:none' id='row_diskon1_" + id + "'>"+res[i]["DISKON1"]+"</td>" +
					"<td style='display:none' id='row_diskon2_" + id + "'>"+res[i]["DISKON2"]+"</td>" +
					"<td style='display:none' id='row_diskon3_" + id + "'>"+res[i]["DISKON3"]+"</td>" +
					"<td style='display:none' id='row_hargapajak_" + id + "' align='right'>" + (parseFloat(res[i]["HARGA_PAJAK"])).formatMoney(2) + "</td>" +
					"<td style='display:none' id='row_harganet_" + id + "' align='right'>" + (parseFloat(res[i]["HARGA_NET"])).formatMoney(2) + "</td>" +
					"<td id='row_hargafinal_" + id + "' align='right'>" + (parseFloat(res[i]["HARGA_FINAL"])).formatMoney(2) + "</td>" +
					"<td><button class='btn btn-xs btn-success' onclick='editPP(\"" + id + "\");'><i class='fa fa-plus fa-fw'></i> Edit</button><button class='btn btn-xs btn-danger' onclick='dialogHapus(\"" + id + "\",\"" + res[i]["BARANG_NAMA"] + "\");'><i class='fa fa-times fa-fw'></i> Hapus</button></td>" +
					"</tr>"
					);
			}
		}
	...
	function ambilNilai(id,idx,type){
		...
		if(pilih_pajak!='3'){
			if(pilih_pajak=='1'){
				...
				harga_awal = harga_barang - harga_pajak;
			} else {
				harga_awal = harga_barang;
				harga_pajak = (pajak/100)*harga_barang;
				harga_final = harga_awal + harga_pajak;
			}
		}
		harga_net = harga_awal;
    ...
    function save(param) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>p-material/po/updatePo",
			data: {PO_ID: <?= $PO["PURCHASE_ORDER_ID"] ?>, RAB_ID: rab_id, SUPPLIER_ID: sup_id, STATUS_PO_ID: stat_po, PAJAK_ID: pajak_id, KATEGORI_PAJAK_ID: kat_pajak_id, GUDANG_ID: gudang_id, TOP_ID: top, KATEGORI_PP_ID: kat_po, PURCHASE_ORDER_TOTAL: poTotal, BARANG: barangString, ALAMAT: alamat_kirim, NAMA_CP: nama_cp, TELP_CP: telp_cp}, 
			cache: false,
			success: function(result){
				if(result=="success") {
