...
class Kategorianalisa extends Plan_Controller {
    ...
    public function index() {
        ...
        $this->title = "Master Kategori Analisa";
        $this->data['listKategorisupplier'] = $this->mKategoriAnalisa->_select(array(mKategoriAnalisa::ACTIVE => 1));
        $this->display('acKategoriAnalisa', $this->data);
    }

    public function delete() {
        $this->mKategoriAnalisa->_delete(array(mKategoriAnalisa::ID => $this->input->post("tobe_deleted_id")), mKategoriAnalisa::ACTIVE);
        redirect(base_url() . "master/kategorianalisa");
    }

    public function update() {
        $id = $this->input->post(mKategoriAnalisa::ID);
        $input= $this->input->post();
        $this->mKategoriAnalisa->_update($input, array(mKategoriAnalisa::ID => $id));
        redirect(base_url() . "master/kategorianalisa");
    }

    public function insert() {
        $input= $this->input->post();
        $input[mKategoriAnalisa::ACTIVE] = 1;
        $this->mKategoriAnalisa->_insert($input);
        redirect(base_url() . "master/kategorianalisa");
    }
