...
class Lokasiupah extends Plan_Controller {
    ...
    public function index() {
        ...
        $this->title = "Master Lokasi Upah";
        $this->data['listKategorisupplier'] = $this->mLokasiUpah->_select(array(mLokasiUpah::ACTIVE => 1));
        $this->display('acLokasiUpah', $this->data);
    }

    public function delete() {
        $this->mLokasiUpah->_delete(array(mLokasiUpah::ID => $this->input->post("tobe_deleted_id")), mLokasiUpah::ACTIVE);
        redirect(base_url() . "master/lokasiupah");
    }

    public function update() {
        $id = $this->input->post(mLokasiUpah::ID);
        $input= $this->input->post();
        $this->mLokasiUpah->_update($input, array(mLokasiUpah::ID => $id));
        redirect(base_url() . "master/lokasiupah");
    }

    public function insert() {
        $input= $this->input->post();
        $input[mLokasiUpah::ACTIVE] = 1;
        $this->mLokasiUpah->_insert($input);
        redirect(base_url() . "master/lokasiupah");
    }
