...
class Gudang extends Plan_Controller {
    ...
    public function index() {
        ...
        $this->data['listGudang'] = $this->mGudang->_select(
                array(mGudang::ACTIVE => '1')
        );
        $this->display('acGudang', $this->data);
    }

    public function delete() {
        $this->mGudang->_delete(array(mGudang::ID => $this->input->post("tobe_deleted_id")), mGudang::ACTIVE);
        redirect(base_url() . "master/gudang");
    }

    public function update() {
        $id = $this->input->post("GUDANG_ID");
        $input= $this->input->post();
        $this->mGudang->_update($input, array(mGudang::ID => $id));
        redirect(base_url() . "master/gudang");
    }

    public function insert() {
        $input= $this->input->post();
        $this->mGudang->_insert($input);
        redirect(base_url() . "master/gudang");
    }
