...
<script>
	...
	function changeRAB(){
		...
		var total = selectValue['<?= mRab::TOTAL ?>'];
		maxOverhead = 0.05*total;
		persenMaterial = selectValue['<?= mRab::MATERIAL ?>']/total;
		persenUpah = selectValue['<?= mRab::UPAH ?>']/total;
		maxMaterial = persenMaterial * maxOverhead;
		maxUpah = persenUpah * maxOverhead;
		if(tipe=="material"){
			data1[disableCellRow[1]][6] = maxMaterial;
			batas = maxMaterial;
		} else {
			data1[disableCellRow[1]][6] = maxUpah;
			batas = maxUpah;
		}
		hot1.render();
	}
			...
			success: function(result){
				if(result!=null) {
					...
					for(var i=0;i<rab.length;i++){
						toAppend += '<option value=\'{"<?= mRab::ID ?>": "' + rab[i]['<?= mRab::ID ?>'] + '","<?= mRab::LOKASI ?>": "' + rab[i]['<?= mRab::LOKASI ?>'] + '","<?= mRab::TOTAL ?>" : "' + parseFloat(rab[i]['<?= mRab::TOTAL ?>']) + '","<?= mRab::MATERIAL ?>" : "' + parseFloat(rab[i]['<?= mRab::MATERIAL ?>']) + '","<?= mRab::UPAH ?>" : "' + parseFloat(rab[i]['<?= mRab::UPAH ?>']) + '"}\' >'+rab[i]['<?= mRab::NAME ?>']+'</option>';
					}
    ...
    <div class="container">
        <div class="row">
							...
							<div class="col-lg-12 col-lg-offset-0">
								<div class="form-horizontal" role="form">
									<div class="form-group">
										...
										<div class="col-lg-5">
											<input type="text" class="form-control" id="<?= mOverhead::KODE ?>" name="<?= mOverhead::KODE ?>" placeholder="kode overhead">
										</div>
									</div>
									<div class="form-group">
			...
			echo "<option value='" . $i[mMainProject::ID] . "'>" . $i[mMainProject::NAMA] . "</option>";
							...
							<div style="width: 100%" align="center"><button id="add" class="btn btn-primary btn-lg" onclick="simpanOverhead()">Simpan</button></div>
						</div>
					</div>
		...
		echo "<option value='" . $i['SATUAN_NAMA'] . "'>" . $i['SATUAN_NAMA'] . "</option>";
								...
								<div class="form-group">
									...
									<div class="col-lg-5" align="center">
										<button type="button" class="btn btn-info" onclick="ambilNilai(0, 0,'subcon')" aria-hidden="true">Tambahkan</button>
...
<script>
	var disableCell = ['Jumlah Total'];
	var disableCellRow = [1,2];
	...
	var data1 = [
	  ...
	  settings1 = {
		...
		columns: [
			...
			{data: 5, readOnly: true, className: "htMiddle"},
		...
		if(currentRow!=-1 && currentCol!=-1){
			...
			if(type=="material"){
				...
				if (!isEmptySubcon()){
					...
					data1[currentRow][1] = $('#subcon_nama').val();
					data1[currentRow][2] = "LS";
					data1[currentRow][3] = 0;
					data1[currentRow][4] = $('#subcon_satuan').val();
					data1[currentRow][5] = $('#subcon_harga').val();
					data1[currentRow][6] = 0;
	...
	function simpanOverhead(){
		...
		var kodeOverhead = $('#<?= mOverhead::KODE ?>').val();
		...
		console.log(dataDetailString);
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>master/overhead/simpanOverhead",
			data: {kode: kodeOverhead, nama: namaOverhead, rab: rabID, detail: dataDetailString, total: totalOverhead}, 
			cache: false,
			success: function(result){
				if(result=="success") 
