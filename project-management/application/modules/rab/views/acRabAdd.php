    ...
    <div class="container">
        <div class="row">
									...
									<div class="form-group">
										...
										<div class="col-lg-4">
											<div class="input-group">
											  <input type="text" class="form-control" id="luas_bangunan" name="luas_bangunan" placeholder="luas bangunan" aria-describedby="basic-addon2">
			...
			echo "<option value='" . $i['LOKASI_UPAH_ID'] . "'>" . $i['LOKASI_UPAH_NAMA'] . "</option>";
							...
							<div style="width: 100%; margin-top: 20px" align="center" id="btn_container"><button id="add" class="btn btn-primary btn-lg" onclick="simpanRAB(<?= $project_id; ?>)">Simpan</button></div>
						</div>
					</div>
...
<script>
	...
	function simpanRAB(project_id){
		...
		for(var i=0; i<rab.length; i++){
			...
			var c = 1;
			for(var j=rab[i]["row_kategori"]+1; j<rab[i]["row_jumlah"]-1; j++){
				analisa.push({ urutan: c, id: data1[j][0], nama: data1[j][2], kode: data1[j][3], volume: parseFloat(data1[j][4]), satuan: data1[j][9], keterangan: data1[j][10 ], harga: parseFloat(data1[j][6]), total: parseFloat(data1[j][7]), tipe: data1[j][8] });
				c++;
			}
		...
		$.ajax({
			...
			data: {kode: kodeRAB, nama: namaRAB, luas: luasRAB, items: rabstring, projectID: project_id, estimatorID: estimator_id, total: rab_total, lokasi: lokasiRAB},
			cache: false,
